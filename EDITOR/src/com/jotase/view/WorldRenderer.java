...
public class WorldRenderer {
	...
	private void gamingState(float h)
	{
		...
		if (this.ball.isWINNER()) {
			...
			if(ball.isCorrect())
			{
				drawPortal();
				{
					if((ball.getPosition().x>port.getPosition().x-40
							&& ball.getPosition().x <port.getPosition().x+40)
							&& (ball.getPosition().y>port.getPosition().y-40
									&& ball.getPosition().y <port.getPosition().y+40))
					{
						controller.isTouch();
					}
	...
	private void addLevel() {
		...
		//	asignCorrectValue();
			...
			font.draw(batch, "GAME OVER", 50, 60);
		}
